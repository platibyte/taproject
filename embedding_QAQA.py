...
def evaluate_ranking(question_embeddings, candidate_embeddings, correct_indices):
    scores = torch.matmul(question_embeddings, candidate_embeddings.T)  # Cosinus-Ähnlichkeiten

    ranks = []
    for i, correct_idx in enumerate(correct_indices):
        ranked_indices = torch.argsort(scores[i], descending=True)  # Kandidaten sortieren
...
# GPT-Antworten evaluieren (Fragen mit GPT-Antworten vergleichen)
gpt_ranks = evaluate_ranking(question_embeddings, gpt_embeddings, correct_indices)
